define('node_modules/monaco-editor/esm/vs/editor/common/controller/cursorTypeOperations', function(require, exports, module) {
  ...
  var shiftCommand_js_1 = require("node_modules/monaco-editor/esm/vs/editor/common/commands/shiftCommand");
  var surroundSelectionCommand_js_1 = require("node_modules/monaco-editor/esm/vs/editor/common/commands/surroundSelectionCommand");
  ...
  var TypeOperations = /** @class */ (function () {
      ...
      TypeOperations._distributedPaste = function (config, model, selections, text) {
          var commands = [];
          for (var i = 0, len = selections.length; i < len; i++) {
              commands[i] = new replaceCommand_js_1.ReplaceCommand(selections[i], text[i]);
          }
          return new cursorCommon_js_1.EditOperationResult(0 /* Other */, commands, {
      ...
      TypeOperations._simplePaste = function (config, model, selections, text, pasteOnNewLine) {
          ...
          for (var i = 0, len = selections.length; i < len; i++) {
              ...
              if (pasteOnNewLine) {
                  ...
                  var typeSelection = new range_js_1.Range(position.lineNumber, 1, position.lineNumber, 1);
                  commands[i] = new replaceCommand_js_1.ReplaceCommand(typeSelection, text);
              }
              else {
                  commands[i] = new replaceCommand_js_1.ReplaceCommand(selection, text);
              }
          }
          return new cursorCommon_js_1.EditOperationResult(0 /* Other */, commands, {
              shouldPushStackElementBefore: true,
      ...
      TypeOperations._goodIndentForLine = function (config, model, lineNumber) {
          var action = null;
          var indentation = '';
          var expectedIndentAction = config.autoIndent ? languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.getInheritIndentForLine(model, lineNumber, false) : null;
          if (expectedIndentAction) {
              ...
              var expectedEnterAction = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.getEnterAction(model, new range_js_1.Range(lastLineNumber, maxColumn, lastLineNumber, maxColumn));
      ...
      TypeOperations._replaceJumpToNextIndent = function (config, model, selection, insertsAutoWhitespace) {
          ...
          if (config.insertSpaces) {
              var visibleColumnFromColumn = cursorCommon_js_1.CursorColumns.visibleColumnFromColumn2(config, model, position);
      ...
      TypeOperations.replacePreviousChar = function (prevEditOperationType, config, model, selections, txt, replaceCharCnt) {
          ...
          for (var i = 0, len = selections.length; i < len; i++) {
              ...
              var range = new range_js_1.Range(pos.lineNumber, startColumn, pos.lineNumber, pos.column);
              commands[i] = new replaceCommand_js_1.ReplaceCommand(range, txt);
          }
          return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, commands, {
      ...
      TypeOperations._enter = function (config, model, keepPosition, range) {
          ...
          var r = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.getEnterAction(model, range);
          if (r) {
              var enterAction = r.enterAction;
              var indentation_2 = r.indentation;
              if (enterAction.indentAction === languageConfiguration_js_1.IndentAction.None) {
                  // Nothing special
                  return TypeOperations._typeCommand(range, '\n' + config.normalizeIndentation(indentation_2 + enterAction.appendText), keepPosition);
              }
              else if (enterAction.indentAction === languageConfiguration_js_1.IndentAction.Indent) {
                  // Indent once
                  return TypeOperations._typeCommand(range, '\n' + config.normalizeIndentation(indentation_2 + enterAction.appendText), keepPosition);
              }
              else if (enterAction.indentAction === languageConfiguration_js_1.IndentAction.IndentOutdent) {
                  // Ultra special
              ...
              else if (enterAction.indentAction === languageConfiguration_js_1.IndentAction.Outdent) {
          ...
          var ir = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.getIndentForEnter(model, range, {
          ...
          if (ir) {
              var oldEndViewColumn = cursorCommon_js_1.CursorColumns.visibleColumnFromColumn2(config, model, range.getEndPosition());
              ...
              if (indentation !== config.normalizeIndentation(ir.beforeEnter)) {
                  ...
                  range = new range_js_1.Range(range.startLineNumber, 1, range.endLineNumber, range.endColumn);
              }
              ...
              if (keepPosition) {
                  return new replaceCommand_js_1.ReplaceCommandWithoutChangingPosition(range, beforeText + config.normalizeIndentation(ir.afterEnter), true);
              }
              else {
                  var offset = 0;
                  ...
                  return new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(range, beforeText + config.normalizeIndentation(ir.afterEnter), 0, offset, true);
              }
          }
          else {
      ...
      TypeOperations._runAutoIndentType = function (config, model, range, ch) {
          var currentIndentation = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.getIndentationAtPosition(model, range.startLineNumber, range.startColumn);
          var actualIndentation = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.getIndentActionForType(model, range, ch, {
          ...
          if (actualIndentation !== config.normalizeIndentation(currentIndentation)) {
              ...
              if (firstNonWhitespace === 0) {
                  return TypeOperations._typeCommand(new range_js_1.Range(range.startLineNumber, 0, range.endLineNumber, range.endColumn), config.normalizeIndentation(actualIndentation) + ch, false);
              }
              else {
                  return TypeOperations._typeCommand(new range_js_1.Range(range.startLineNumber, 0, range.endLineNumber, range.endColumn), config.normalizeIndentation(actualIndentation) +
      ...
      TypeOperations._isAutoClosingCloseCharType = function (config, model, selections, ch) {
          var autoCloseConfig = cursorCommon_js_1.isQuote(ch) ? config.autoClosingQuotes : config.autoClosingBrackets;
      ...
      TypeOperations._runAutoClosingCloseCharType = function (prevEditOperationType, config, model, selections, ch) {
          ...
          for (var i = 0, len = selections.length; i < len; i++) {
              var selection = selections[i];
              var position = selection.getPosition();
              var typeSelection = new range_js_1.Range(position.lineNumber, position.column, position.lineNumber, position.column + 1);
      ...
      TypeOperations._isAutoClosingOpenCharType = function (config, model, selections, ch) {
          var chIsQuote = cursorCommon_js_1.isQuote(ch);
          ...
          for (var i = 0, len = selections.length; i < len; i++) {
              ...
              try {
                  shouldAutoClosePair = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.shouldAutoClosePair(ch, lineTokens, position.column);
              }
              catch (e) {
      ...
      TypeOperations._runAutoClosingOpenCharType = function (prevEditOperationType, config, model, selections, ch) {
          ...
          for (var i = 0, len = selections.length; i < len; i++) {
              ...
              commands[i] = new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(selection, ch + closeCharacter, 0, -closeCharacter.length);
          }
          return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, commands, {
              shouldPushStackElementBefore: true,
              shouldPushStackElementAfter: false
      ...
      TypeOperations._shouldSurroundChar = function (config, ch) {
          if (cursorCommon_js_1.isQuote(ch)) {
      ...
      TypeOperations._isSurroundSelectionType = function (config, model, selections, ch) {
          ...
          var isTypingAQuoteCharacter = cursorCommon_js_1.isQuote(ch);
          for (var i = 0, len = selections.length; i < len; i++) {
              var selection = selections[i];
              if (selection.isEmpty()) {
                  return false;
              ...
              if (isTypingAQuoteCharacter && selection.startLineNumber === selection.endLineNumber && selection.startColumn + 1 === selection.endColumn) {
                  ...
                  if (cursorCommon_js_1.isQuote(selectionText)) {
      ...
      TypeOperations._typeInterceptorElectricChar = function (prevEditOperationType, config, model, selection, ch) {
          ...
          var electricAction;
          try {
              electricAction = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.onElectricCharacter(ch, lineTokens, position.column);
          }
          catch (e) {
              errors_js_1.onUnexpectedError(e);
              return null;
          ...
          if (electricAction.appendText) {
              var command = new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(selection, ch + electricAction.appendText, 0, -electricAction.appendText.length);
              return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, [command], {
          ...
          if (electricAction.matchOpenBracket) {
              ...
              if (match) {
                  ...
                  var typeSelection = new range_js_1.Range(position.lineNumber, 1, position.lineNumber, position.column);
                  var command = new replaceCommand_js_1.ReplaceCommand(typeSelection, typeText);
                  return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, [command], {
                      shouldPushStackElementBefore: false,
                      shouldPushStackElementAfter: true
                  });
      ...
      TypeOperations.compositionEndWithInterceptors = function (prevEditOperationType, config, model, selections) {
          ...
          for (var i = 0; i < selections.length; i++) {
              ...
              if (config.autoClosingPairsOpen.hasOwnProperty(ch)) {
                  if (cursorCommon_js_1.isQuote(ch) && position.column > 2) {
                      var wordSeparators = wordCharacterClassifier_js_1.getMapForWordSeparators(config.wordSeparators);
                  ...
                  if (characterAfter) {
                      var isBeforeCloseBrace = TypeOperations._isBeforeClosingBrace(config, ch, characterAfter);
                      var shouldAutoCloseBefore = cursorCommon_js_1.isQuote(ch) ? config.shouldAutoCloseBefore.quote : config.shouldAutoCloseBefore.bracket;
                  ...
                  var shouldAutoClosePair = false;
                  try {
                      shouldAutoClosePair = languageConfigurationRegistry_js_1.LanguageConfigurationRegistry.shouldAutoClosePair(ch, lineTokens, position.column - 1);
                  }
                  catch (e) {
                      errors_js_1.onUnexpectedError(e);
                  }
                  if (shouldAutoClosePair) {
                      var closeCharacter = config.autoClosingPairsOpen[ch];
                      commands[i] = new replaceCommand_js_1.ReplaceCommandWithOffsetCursorState(selections[i], closeCharacter, 0, -closeCharacter.length);
                  }
              }
          }
          return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, commands, {
      ...
      TypeOperations.typeWithInterceptors = function (prevEditOperationType, config, model, selections, ch) {
          if (ch === '\n') {
              ...
              return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, commands_1, {
                  shouldPushStackElementBefore: true,
                  shouldPushStackElementAfter: false,
              });
          }
          if (this._isAutoIndentType(config, model, selections)) {
              ...
              if (!autoIndentFails) {
                  return new cursorCommon_js_1.EditOperationResult(1 /* Typing */, commands_2, {
                      shouldPushStackElementBefore: true,
                      shouldPushStackElementAfter: false,
                  });
              }
          ...
          var commands = [];
          for (var i = 0, len = selections.length; i < len; i++) {
              commands[i] = new replaceCommand_js_1.ReplaceCommand(selections[i], ch);
          }
