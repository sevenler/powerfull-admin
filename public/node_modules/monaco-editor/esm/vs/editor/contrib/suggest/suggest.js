define('node_modules/monaco-editor/esm/vs/editor/contrib/suggest/suggest', function(require, exports, module) {
  ...
  exports.Context = {
      Visible: new contextkey_js_1.RawContextKey('suggestWidgetVisible', false),
      MultipleSuggestions: new contextkey_js_1.RawContextKey('suggestWidgetMultipleSuggestions', false),
      MakesTextEdit: new contextkey_js_1.RawContextKey('suggestionMakesTextEdit', true),
      AcceptSuggestionsOnEnter: new contextkey_js_1.RawContextKey('acceptSuggestionOnEnter', true)
  };
  var CompletionItem = /** @class */ (function () {
      function CompletionItem(position, completion, container, provider, model) {
          ...
          // sorting, filtering
          this.score = filters_js_1.FuzzyScore.Default;
          this.distance = 0;
              ...
              this.resolve = function (token) {
                  if (!cached_1) {
                      ...
                      cached_1 = Promise.resolve(resolveCompletionItem.call(provider, model, position, completion, token)).then(function (value) {
                          objects_js_1.assign(completion, value);
                          isDone_1 = true;
                      }, function (err) {
  ...
  function provideSuggestionItems(model, position, options, context, token) {
      ...
      if (token === void 0) {
          token = cancellation_js_1.CancellationToken.None;
      }
      var allSuggestions = [];
      var wordUntil = model.getWordUntilPosition(position);
      var defaultRange = new range_js_1.Range(position.lineNumber, wordUntil.startColumn, position.lineNumber, wordUntil.endColumn);
      ...
      var result = async_js_1.first(factory, function () {
          // stop on result or cancellation
          return hasResult || token.isCancellationRequested;
      }).then(function () {
          if (token.isCancellationRequested) {
              return Promise.reject(errors_js_1.canceled());
          }
  ...
  editorExtensions_js_1.registerDefaultLanguageCommand('_executeCompletionItemProvider', function (model, position, args) {
      var result = {
          incomplete: false,
          suggestions: []
      };
      ...
      return provideSuggestionItems(model, position).then(function (items) {
          for (var _i = 0, items_1 = items; _i < items_1.length; _i++) {
              ...
              if (resolving.length < maxItemsToResolve) {
                  resolving.push(item.resolve(cancellation_js_1.CancellationToken.None));
              }
