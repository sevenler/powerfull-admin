define('node_modules/monaco-editor/esm/vs/base/browser/ui/splitview/splitview', function(require, exports, module) {
  ...
  ''/*@require node_modules/monaco-editor/esm/vs/base/browser/ui/splitview/splitview.css*/;
  ...
  var SplitView = /** @class */ (function (_super) {
      ...
      function SplitView(container, options) {
          ...
          _this._onDidSashChange = _this._register(new event_js_1.Emitter());
          _this.onDidSashChange = _this._onDidSashChange.event;
          _this._onDidSashReset = _this._register(new event_js_1.Emitter());
      ...
      SplitView.prototype.addView = function (view, size, index) {
          ...
          var containerDisposable = lifecycle_js_1.toDisposable(function () { return _this.viewContainer.removeChild(container); });
          var disposable = lifecycle_js_1.combinedDisposable([onChangeDisposable, containerDisposable]);
          ...
          if (this.viewItems.length > 1) {
              ...
              var sash_1 = new sash_js_1.Sash(this.sashContainer, layoutProvider, {
              ...
              var onStart = event_js_1.Event.map(sash_1.onDidStart, sashEventMapper);
              var onStartDisposable = onStart(this.onSashStart, this);
              var onChange = event_js_1.Event.map(sash_1.onDidChange, sashEventMapper);
              var onChangeDisposable_1 = onChange(this.onSashChange, this);
              var onEnd = event_js_1.Event.map(sash_1.onDidEnd, function () { return arrays_js_1.firstIndex(_this.sashItems, function (item) { return item.sash === sash_1; }); });
              var onEndDisposable = onEnd(this.onSashEnd, this);
              var onDidResetDisposable = sash_1.onDidReset(function () { return _this._onDidSashReset.fire(arrays_js_1.firstIndex(_this.sashItems, function (item) { return item.sash === sash_1; })); });
              var disposable_1 = lifecycle_js_1.combinedDisposable([onStartDisposable, onChangeDisposable_1, onEndDisposable, onDidResetDisposable, sash_1]);
      ...
      SplitView.prototype.layout = function (size) {
          ...
          this.size = size;
          if (!this.proportions) {
              var indexes = arrays_js_1.range(this.viewItems.length);
              var lowPriorityIndexes = indexes.filter(function (i) { return _this.viewItems[i].view.priority === 1 /* Low */; });
              var highPriorityIndexes = indexes.filter(function (i) { return _this.viewItems[i].view.priority === 2 /* High */; });
              this.resize(this.viewItems.length - 1, size - previousSize, undefined, lowPriorityIndexes, highPriorityIndexes);
          }
          else {
              for (var i = 0; i < this.viewItems.length; i++) {
                  var item = this.viewItems[i];
                  item.size = numbers_js_1.clamp(Math.round(this.proportions[i] * size), item.view.minimumSize, item.view.maximumSize);
              }
          }
          this.distributeEmptySpace();
          this.layoutViews();
      ...
      SplitView.prototype.onSashStart = function (_a) {
          ...
          var index = arrays_js_1.firstIndex(this.sashItems, function (item) { return item.sash === sash; });
          // This way, we can press Alt while we resize a sash, macOS style!
          var disposable = lifecycle_js_1.combinedDisposable([
              event_js_2.domEvent(document.body, 'keydown')(function (e) { return resetSashDragState(_this.sashDragState.current, e.altKey); }),
              event_js_2.domEvent(document.body, 'keyup')(function () { return resetSashDragState(_this.sashDragState.current, false); })
          ]);
      ...
      SplitView.prototype.onViewChange = function (item, size) {
          ...
          size = numbers_js_1.clamp(size, item.view.minimumSize, item.view.maximumSize);
      ...
      SplitView.prototype.resizeView = function (index, size) {
          ...
          if (delta !== 0 && index < this.viewItems.length - 1) {
              var downIndexes = arrays_js_1.range(index + 1, this.viewItems.length);
              ...
              var deltaDown = numbers_js_1.clamp(delta, -expandDown, collapseDown);
              this.resize(index, deltaDown);
              delta -= deltaDown;
          }
          if (delta !== 0 && index > 0) {
              var upIndexes = arrays_js_1.range(index - 1, -1);
              ...
              var deltaUp = numbers_js_1.clamp(-delta, -collapseUp, expandUp);
      ...
      SplitView.prototype.resize = function (index, delta, sizes, lowPriorityIndexes, highPriorityIndexes, overloadMinDelta, overloadMaxDelta) {
          var _this = this;
          if (sizes === void 0) {
              sizes = this.viewItems.map(function (i) { return i.size; });
          }
          ...
          var upIndexes = arrays_js_1.range(index, -1);
          var downIndexes = arrays_js_1.range(index + 1, this.viewItems.length);
          if (highPriorityIndexes) {
              for (var _i = 0, highPriorityIndexes_1 = highPriorityIndexes; _i < highPriorityIndexes_1.length; _i++) {
                  ...
                  arrays_js_1.pushToStart(upIndexes, index_1);
                  arrays_js_1.pushToStart(downIndexes, index_1);
              }
          }
          if (lowPriorityIndexes) {
              for (var _a = 0, lowPriorityIndexes_1 = lowPriorityIndexes; _a < lowPriorityIndexes_1.length; _a++) {
                  ...
                  arrays_js_1.pushToEnd(upIndexes, index_2);
                  arrays_js_1.pushToEnd(downIndexes, index_2);
              }
          }
          ...
          delta = numbers_js_1.clamp(delta, minDelta, maxDelta);
          for (var i = 0, deltaUp = delta; i < upItems.length; i++) {
              var item = upItems[i];
              var size = numbers_js_1.clamp(upSizes[i] + deltaUp, item.view.minimumSize, item.view.maximumSize);
          ...
          for (var i = 0, deltaDown = delta; i < downItems.length; i++) {
              ...
              var size = numbers_js_1.clamp(downSizes[i] - deltaDown, item.view.minimumSize, item.view.maximumSize);
      ...
      SplitView.prototype.distributeEmptySpace = function () {
          ...
          for (var i = this.viewItems.length - 1; emptyDelta !== 0 && i >= 0; i--) {
              var item = this.viewItems[i];
              var size = numbers_js_1.clamp(item.size + emptyDelta, item.view.minimumSize, item.view.maximumSize);
